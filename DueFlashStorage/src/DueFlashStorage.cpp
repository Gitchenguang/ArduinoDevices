...
boolean DueFlashStorage::put(uint32_t address, void* data , uint32_t dataLength ){
	byte buff[ dataLength ];
	memcpy( buff , data , dataLength );
	write( address , buff, dataLength );
}
boolean DueFlashStorage::get(uint32_t address, void* data , uint32_t dataLength ){
	byte* buff = readAddress(address);
	memcpy( data , buff , dataLength );
}
